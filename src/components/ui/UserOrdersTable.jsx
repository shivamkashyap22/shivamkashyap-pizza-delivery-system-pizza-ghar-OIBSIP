...
function UserOrdersTable({ orders }) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      default:
        return (
          <FaBoxOpen className="inline-flex items-center justify-center text-yellow-600" />
        );
    }
  };
  ...
  return (
    <>
      <div className="overflow-x-auto w-full">
        <table className="bg-yellow-400 w-full table-auto border-collapse border-2 border-yellow-500 rounded-2xl text-center overflow-hidden whitespace-no-wrap">
          <thead className="bg-yellow-400 h-10 uppercase font-bold text-white">
            <tr>
              <th>Order ID</th>
              <th>User ID</th>
              <th>Total Price</th>
          ...
          <tbody className="bg-yellow-100 text-yellow-500">
            {orders.slice(startIndex, endIndex).map((order) => (
              <tr key={order._id}>
                <td className="border border-yellow-500 px-4 py-2 sm:px-2 sm:py-1">
                  {order._id}
                </td>
                <td className="border border-yellow-500 px-4 py-2 sm:px-2 sm:py-1">
                  {order.user}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center my-4">
        ...
        <FaChevronRight className="hidden sm:inline-block text-yellow-400 mx-2" />
        <FaChevronDown className="sm:hidden text-yellow-400 mx-2" />
